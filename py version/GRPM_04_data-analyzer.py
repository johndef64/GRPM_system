...
directory_11 = 'grpm_random/grpm_random_'+db_tag+'_'+'04'
directory_12 = 'grpm_random/grpm_random_'+db_tag+'_'+'05'
directory_13 = 'grpm_random/grpm_random_'+db_tag+'_'+'06'
directory_14 = 'grpm_random/grpm_random_'+db_tag+'_'+'07'
directory_15 = 'grpm_random/grpm_random_'+db_tag+'_'+'08'
directory_16 = 'grpm_random/grpm_random_'+db_tag+'_'+'09'
directory_17 = 'grpm_random/grpm_random_'+db_tag+'_'+'00'
directory_18 = 'grpm_random/grpm_random_'+db_tag+'_'+'norep1'
directory_19 = 'grpm_random/grpm_random_'+db_tag+'_'+'norep2'
directory_20 = 'grpm_random/grpm_random_'+db_tag+'_'+'norep3'
directory_21 = 'grpm_random/grpm_random_'+db_tag+'_'+'norep4'
directory_22 = 'grpm_random/grpm_random_'+db_tag+'_'+'norep5'
directory_23 = 'grpm_random/grpm_random_'+db_tag+'_'+'norep6'
directory_24 = 'grpm_random/grpm_random_'+db_tag+'_'+'norep7'
directory_25 = 'grpm_random/grpm_random_'+db_tag+'_'+'norep8'
directory_26 = 'grpm_random/grpm_random_'+db_tag+'_'+'norep9'
directory_27 = 'grpm_random/grpm_random_'+db_tag+'_'+'norep10'
directory_28 = 'grpm_random/grpm_random_'+db_tag+'_'+'norep11'
...
#Visualize mesh enrichment:
#choose mesh to get rid eventually
check = 1 # <= master_df index
...
# # Analyse GRPMX Report Data

# ## Create Stat datasheet with all Reports + grpm data
...
comp_df_stat = pd.DataFrame()
comp_df_stat = pd.DataFrame({'dir':directories,
                              'int_mean'           :0,
                              'mesh_mean'          :0,
...
for i in range(len(comp_df_stat)):
    repost = repo_int_sort_list[i]
    grpm_out = grpm_out_list[i]
    comp_df_stat.loc[i, 'int_mean' ] = round(repost[value  ].iloc[  f_gene:l_gen2].mean(), cifr+2)
    comp_df_stat.loc[i, 'mesh_mean'] = round(repost[value_2].iloc[f_gene:l_gen2].mean(), cifr)
    comp_df_stat.loc[i, 'mesh_ref_ratio_mean'] = round(repost[value_x].iloc[f_gene:l_gen2].mean(), cifr+2)
    comp_df_stat.loc[i, 'pmids_mean'         ] = round(repost[value_3].iloc[f_gene:l_gen2].mean(), cifr)
    comp_df_stat.loc[i, 'pmids_ratio_mean'   ] = round(repost[value_4].iloc[f_gene:l_gen2].mean(), cifr)
    comp_df_stat.loc[i, 'grpm_'+out_1  ] = grpm_out[out_1].nunique()
    comp_df_stat.loc[i, 'grpm_'+out_r  ] = grpm_out[out_r].nunique()
    comp_df_stat.loc[i, 'grpm_'+out_2  ] = grpm_out[out_2].nunique()
    comp_df_stat.loc[i, 'grpm_'+out_3  ] = grpm_out[out_3].nunique()

#comp_df_stat[[out_1,out_2,out_3]] = comp_df_stat[[out_1,out_2,out_3]].astype(int)
print('Survey datasheet - gene mean values for InterestIndex Top',l_gen2,'genes')
comp_df_stat#.to_csv('nutri+neuro+17random.csv')
#pyperclip.copy(comp_df_stat.to_csv())
...
comp_df_stat_all = pd.DataFrame({'dir':directories,
                                 'int_mean'           :0,
                                 'mesh_mean'          :0,
                                 'mesh_ref_ratio_mean':0,
                                 'pmids_mean'         :0,
...
for i in range(len(comp_df_stat_all)):
    repost = repo_int_sort_list[i]
    grpm_out = grpm_out_list[i]
    comp_df_stat_all.loc[i, 'int_mean' ] = round(repost[value].iloc[  f_gene:l_gen2].mean(), cifr+2)
    comp_df_stat_all.loc[i, 'mesh_mean'] = round(repost[value_2].iloc[f_gene:l_gen2].mean(), cifr)
    comp_df_stat_all.loc[i, 'mesh_ref_ratio_mean'] = round(repost[value_x].iloc[f_gene:l_gen2].mean(), cifr+2)
    ...
    comp_df_stat_all.loc[i, 'grpm_'+out_1  ] = grpm_out[out_1].nunique()
    comp_df_stat_all.loc[i, 'grpm_'+out_r  ] = grpm_out[out_r].nunique()
    comp_df_stat_all.loc[i, 'grpm_'+out_2  ] = grpm_out[out_2].nunique()
    comp_df_stat_all.loc[i, 'grpm_'+out_3  ] = grpm_out[out_3].nunique()

add_label = True
if add_label == True:
    #titles = master_df['label']
    comp_df_stat_all['label'] = master_df['label']
    columns = comp_df_stat_all.columns.tolist()
    columns = [columns[-1]] + columns[:-1]
    comp_df_stat_all = comp_df_stat_all[columns]

comp_df_stat_all[['grpm_'+out_1,
              'grpm_'+out_r,
              'grpm_'+out_2,
              'grpm_'+out_3 ]] = comp_df_stat_all[['grpm_'+out_1,
                                               'grpm_'+out_r,
                                               'grpm_'+out_2,
                                               'grpm_'+out_3 ]].astype(int)
print('Survey datasheet - gene mean values for InterestIndex Top',l_gen2,'genes')
comp_df_stat_all#.to_csv('nutri+neuro+17random.csv')
...
comp_df_stat_all[['dir','percentile_95','genes_95', 'int_min_95']]
...
f_gene, l_gene = 200, 15500 # set top gene survey_l
...
for dat, col, dir  in zip(datas[0:survey_l],
    ...
    #print('int min',dat.min(),col, dir)

plt.axvline(x = 0.005, color= 'red', linestyle='--', label= 'threshold')
#plt.xscale('log')
plt.title('Ig = (Pm/P)g')
...
plt.xlim(0)
plt.legend()
print('\nGRPMX interest index plot:', directory)
...
#plt.xscale('log')
plt.title('Interest index plot for survey VS random, top '+str(l_gene)+' genes')
plt.xscale('log')
#plt.ylim(0, 25)
plt.xlim(10 ** -2.3)
plt.legend()
...
# burn all dataframes on master ref
genes_l = 50 #set master top genes
n = 9 #choose master gene ref from master_df
...
#Boxplotting (trial 'out of frame')
...
# I share a summary of the results obtained so far in the form of a scatter plot.
# 
# The results of the comparison of several mesh lists on the GRPM Database created on the basis of LitVar and Pubmed data:
# In red I plotted the "interest_indexes" of the Top 200 genes of the nutritional mesh list sorted in descending order
# In green the values for the same 200 genes but using the neurological term list.
# In grey the values obtained using 17 random mesh lists.
# 
# GRPMX interest index plot:
# directory colour code
# grpm_survey_nutri red
# grpm_survey_neuro green
# 
# and in logarithmic scale
# 
# comparison example: green (neuro) dots that stand out with high values mean that genes are more interesting neurologically than nutritionally
...
# ### Defining pmid number threshold (GRPM DB)
...
# ## Apply ‘interest index’ threshold to grpm
...
for i in range(len(comp_df_stat_all)):
    ...
    comp_df_stat_all.loc[i, 'genes_thr0.0055'] = repo.gene.nunique()

comp_df_stat_all['genes_thr0.0055'] = comp_df_stat_all['genes_thr0.0055'].astype(int)
comp_df_stat_all
...
if 1>2:
    ...
    comp_df_stat_all[0:10][['label','grpm_gene','genes_thr0.0055','genes_95','grpm_genes_95','percentile_95']]#.to_clipboard()
else:
    display(comp_df_stat_all[11:28][['label','dir','grpm_mesh','grpm_gene','genes_thr0.0055','genes_95','percentile_95']])
    comp_df_stat_all[11:28][['label','grpm_mesh','grpm_pmids','grpm_rsid','grpm_gene','genes_thr0.0055','genes_95','percentile_95']].to_clipboard()
    #comp_df_stat_all[0:10][['label','grpm_mesh','grpm_pmids','grpm_rsid','grpm_gene','genes_thr0.0055','genes_95','percentile_95']].to_clipboard()
...
grpm_out_int_sort[7][['gene','interest_index']].drop_duplicates()
#grpm_out_list[7][['gene']].drop_duplicates()


# master_df.directories[0:survey_l]
...
master_df[['directories','grpm_genes']]
comp_df_stat_all[['dir','grpm_gene','genes_thr0.0055']]


# In[ ]:


# single grpm search (nutri genral excluded):
...
# multi grpm search (nutri general excluded):
grpm_out_int_sort_sub = grpm_out_int_sort[:survey_l]
...
print(filtered_df.gene.nunique(),'/',grpm_out_thre_sub[m].gene.nunique())
...
# multi grpm search (nutri genral excluded):
grpm_out_thre_sub = grpm_out_thre[:survey_l]
...
# Uniques genes thr 0.0055** / total  :
...
# # GRPM analyzer (previous version)

# ## Import nbib data from Database
...
# ## Import grpm data from Database
# 
